...
if [ -z "$TOKEN" ]; then
  ...
  # Колонка date должна вернуться календарной датой. Драйвер по умолчанию
  # разбирает её в объект Date, и в ответе оказывается метка времени -
  # устройство такую запись не принимает, а обмен выглядит исправным.
  check "local_date вернулась датой, а не меткой времени" "\"local_date\":\"$DAY\"" "$out"
